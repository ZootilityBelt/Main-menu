...
function handleInput() {
    ...
    const matches = reptiles.filter(r =>
    ...
    if (matches.length > 0) {
        matches.forEach(match => {
            ...
            // Highlight matching part logic could be added here, but simple text is fine for now
            div.innerHTML = `${match.common} <span style="font-size: 0.8em; color: #888;">(${match.sci})</span>`;
            div.addEventListener("click", () => {
                searchInput.value = match.common; // Or decide whether to use common or sci
                suggestionsBox.classList.remove("show");
                performSearch(match.common); // Auto-search on selection? User said "for the other logic will do after it clicked" referring to search button, but usually selection implies search. 
                // Let's assume they want the result immediately on selection.
            });
            suggestionsBox.appendChild(div);
        });
    } else {
        const div = document.createElement("div");
        div.classList.add("suggestion-no-match");
...
function performSearch(query) {
    ...
    // Exact match preference or find first includes
    const found = reptiles.find(r =>
        r.sci.toLowerCase() === input ||
        r.common.toLowerCase() === input ||
        r.sci.toLowerCase().includes(input) ||
        r.common.toLowerCase().includes(input)
    );
...
function updateScale(zoneText) {
    const pointer = document.getElementById("scalePointer");
    pointer.classList.remove("zone-1", "zone-2", "zone-3", "zone-4");

    // Parse zone (handles "Zone 1", "Zone 1–2", etc.)
    const zones = zoneText.match(/\d/g).map(Number);
    let position = 0;

    if (zones.length === 1) {
        // Single zone: center of that segment
        // Segments: Z1 (0-25%), Z2 (25-50%), Z3 (50-75%), Z4 (75-100%)
        position = (zones[0] - 1) * 25 + 12.5;
        pointer.classList.add("zone-" + zones[0]);
    } else if (zones.length === 2) {
        // Range of zones: center between first and last
        const startPos = (zones[0] - 1) * 25 + 12.5;
        const endPos = (zones[zones.length - 1] - 1) * 25 + 12.5;
        position = (startPos + endPos) / 2;

        // Pick class of the higher zone for the color or keep neutral? 
        // Let's use the higher zone for color
        pointer.classList.add("zone-" + zones[zones.length - 1]);
    }

    pointer.style.left = `calc(${position}% - 2px)`; // -2px to account for half of pointer width (4px)
}

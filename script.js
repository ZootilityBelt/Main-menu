...
// Uses THREE and POSTPROCESSING from global scope (loaded via script tags)
...
class PixelBlast {
    constructor(containerId, options = {}) {
        ...
        if (!this.container) {
            console.error('PixelBlast: Container not found');
            return;
        }
    ...
    init() {
        ...
        if (this.options.noiseAmount > 0) {
            ...
            const noiseEffect = new POSTPROCESSING.Effect('NoiseEffect',
                `uniform float uTime; uniform float uAmount; float hash(vec2 p){ return fract(sin(dot(p, vec2(127.1,311.7))) * 43758.5453);} void mainUv(inout vec2 uv){} void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){ float n=hash(floor(uv*vec2(1920.0,1080.0))+floor(uTime*60.0)); float g=(n-0.5)*uAmount; outputColor=inputColor+vec4(vec3(g),0.0);} `,
                {
                    uniforms: new Map([['uTime', new THREE.Uniform(0)], ['uAmount', new THREE.Uniform(this.options.noiseAmount)]])
                }
            );
            ...
            if (this.composer && this.composer.passes.length > 0) {
                this.composer.passes.forEach(p => { if (p !== noisePass) p.renderToScreen = false; });
            }
        ...
        // Event Listeners
        this.handleResize = this.setSize.bind(this);
    ...
    animate() {
        ...
        if (this.liquidEffect) this.liquidEffect.uniforms.get('uTime').value = this.uniforms.uTime.value;
...
document.addEventListener('DOMContentLoaded', () => {
    const containerId = 'pixel-blast';
    const container = document.getElementById(containerId);

    if (container) {
        new PixelBlast(containerId, {
            variant: "circle",
            pixelSize: 6,
            color: "#B19EEF",
            patternScale: 3,
